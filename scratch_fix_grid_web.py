          ...
          <div className="flex items-start justify-between gap-2 mb-3">
            ...
            {precio && (
              <div className="text-right shrink-0">
                <span className="text-[9px] font-semibold uppercase" style={{ color: '#94a3b8' }}>{LISTAS.find(l => l.id === listaPrecioId)?.nombre} Gs.</span>
...
function TarjetaProducto({ producto: p, onNeedSession }: { producto: RimecAgrupado; onNeedSession: () => void }) {
  ...
  const { activa, carrito, agregarCaja, quitarCaja } = useSesion()
  const listaPrecioId = useSesion(s => s.listaPrecioId)
            ...
            {!activa ? (
              ...
              <div>
                <span className="text-[9px] font-semibold uppercase tracking-wide" style={{ color: '#94a3b8' }}>
                  {LISTAS.find(l => l.id === listaPrecioId)?.nombre} Gs.
                </span>
                {' '}
...
print("Updated CatalogoGrid.tsx with correct fields and store")
