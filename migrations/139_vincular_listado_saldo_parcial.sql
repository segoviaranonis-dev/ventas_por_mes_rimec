-- MIG-139 — Vincular listado con ventas parciales en calle.
-- Regla: filas PPD con saldo > 0 reciben snapshot del nuevo evento;
--        filas 100 % vendidas (saldo = 0) conservan precio_lpn / listado congelado.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'filas_congeladas_venta', v_congeladas,
    'actualizados', v_paso1 + v_paso2,
    'detail', 'SNAPSHOT_OK_MIG139_SALDO_PARCIAL'
  );
END;
$function$;

COMMENT ON FUNCTION public.vincular_listado_a_pp(bigint, bigint, bigint) IS
  'MIG-139: snapshot precios PPD solo filas con saldo > 0. Filas 100% vendidas conservan LP congelado.';

COMMIT;

SELECT 'MIG-139 OK: vincular_listado_a_pp saldo parcial' AS estado;
