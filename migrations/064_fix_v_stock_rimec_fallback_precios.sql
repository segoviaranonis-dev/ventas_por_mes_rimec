-- 064 — HOTFIX: Fallback precios por SKU cuando PP no tiene IC o evento_id
-- OT: OT-RIMEC-WEB-TARJETAS-MULTI-ORIGEN-001 / HOTFIX catalog precios vacíos
--
-- Problema: Si id_intencion_compra es null, la vista caía en el evento cerrado
-- global más nuevo, pero ese evento solo contiene precios de una porción del stock.
-- Fix: Buscar el evento cerrado más nuevo que posea precios para el SKU específico.
...
COMMENT ON VIEW public.v_stock_rimec IS
  'Catálogo web: tránsito PP en ABIERTO/ENVIADO con saldo. Fallback precios por SKU (064).';
