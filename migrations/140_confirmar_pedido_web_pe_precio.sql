-- MIG-140 — confirmar_pedido_web: validar precio PE en stock_pronta_entrega (no v_stock_rimec)
-- Fix: Det N perdió precio — ítems PE legacy det_id < 800M o lote origen_pe
-- Base: MIG-139

BEGIN;

ALTER TABLE public.factura_interna
  ALTER COLUMN pp_id DROP NOT NULL;
...
BEGIN
  ...
  FOR v_lote IN SELECT * FROM jsonb_array_elements(p_payload->'lotes') LOOP
    ...
    FOR v_factura IN SELECT * FROM jsonb_array_elements(v_lote->'facturas') LOOP
      ...
      -- MIG-139: PE manda marca_id=0 cuando linea.marca_id es NULL
      ...
      FOR v_item IN SELECT * FROM jsonb_array_elements(v_factura->'items') LOOP
        ...
        v_is_pe_item := v_is_pe_lote
                        OR (v_det_id IS NOT NULL AND v_det_id >= c_pe_det_base);

        IF v_is_pe_item AND v_det_id IS NOT NULL THEN
          v_pe_stock_id := CASE
            WHEN v_det_id >= c_pe_det_base THEN v_det_id - c_pe_det_base
            ELSE v_det_id
          END;
        ELSE
          v_pe_stock_id := NULL;
        ...
        IF v_is_pe_item AND v_precio_payload IS NOT NULL THEN
          ...
          IF v_precio_bd_lista IS NULL THEN
            RETURN jsonb_build_object(
              'success', false,
              'error', format(
                'Stock PE id=%s no existe. Vacá el carrito y volvé a cargar desde catálogo.',
          ...
          IF v_precio_bd_lista IS DISTINCT FROM v_precio_payload THEN
            RAISE EXCEPTION 'Precio cambió para PE stock % (lista LPN): payload %, BD %.',
        ...
        INSERT INTO public.factura_interna_detalle (
          ...
          v_fi_id,
          CASE WHEN v_is_pe_item THEN NULL ELSE v_det_id END,
          v_cajas, v_pares,
        ...
        IF v_is_pe_item AND v_pares > 0 THEN
          IF v_pe_stock_id IS NULL THEN
            RAISE EXCEPTION 'Ítem PE sin stock id (det %).', COALESCE(v_det_id::text, '?');
          END IF;
          ...
          IF NOT FOUND THEN
            RAISE EXCEPTION 'Stock PE insuficiente id=% (det %).', v_pe_stock_id, v_det_id;
          END IF;
...
COMMENT ON FUNCTION public.confirmar_pedido_web IS
  'MIG-140: PE precio stock_pronta_entrega · marca_id saneado · pp_id NULL PE';

COMMIT;

SELECT 'MIG-140 OK: confirmar_pedido_web PE precio' AS estado;
