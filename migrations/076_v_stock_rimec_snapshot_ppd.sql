-- MIG-076 — v_stock_rimec lee snapshot persistido en PPD (sin JOIN a precio_lista)
...
SELECT DISTINCT ON (ppd.id)
  ...
  ppd.descp_caso_snapshot AS descp_caso,
  ppd.listado_precio_id,
  ppd.precio_dolar_origen,
...
LEFT JOIN public.grupo_estilo_v2 ge ON ge.id_grupo_estilo = COALESCE(lr.grupo_estilo_id, x.cast_style_id)
LEFT JOIN public.tipo_1 t1 ON t1.id_tipo_1 = lr.tipo_1_id
...
COMMENT ON VIEW public.v_stock_rimec IS
  'MIG-076: precios y caso desde snapshot en pedido_proveedor_detalle. Sin JOIN a precio_lista ni biblioteca en runtime.';
