-- MIG-148 — v_stock_pe_rimec: imagen dual proveedor 654/638 + metadatos proveedor
-- Rama 654 calzado: linea-referencia-material-color.jpg
-- Rama 638 Kyly:     linea_color.jpg (strip K inicial en color)
...
SELECT
  ...
  CASE
    WHEN pp.proveedor_importacion_id = 638
     ...
     AND NULLIF(btrim(ppd.color_code), '') IS NOT NULL
    THEN 'https://extrlcvcgypwazxipvqm.supabase.co/storage/v1/object/public/productos/'
         || btrim(ppd.linea) || '_'
         || regexp_replace(btrim(ppd.color_code), '^[Kk]', '') || '.jpg'
...
COMMENT ON VIEW public.v_stock_pe_rimec IS
  'Pronta entrega RIMEC · PPD Alejandro Magno MIG-140/142/143/144/148 · imagen dual 654/638';
