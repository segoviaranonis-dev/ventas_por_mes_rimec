-- ============================================================================
-- MIGRACIÓN 109: Filtrar v_stock_rimec por estado_transito = EN_TRANSITO
-- AUTOR: Héctor & Claude AI
-- FECHA: 2026-06-04
-- PROPÓSITO: CRÍTICO - Solo mostrar PP en tránsito, NO los que ya están en depósito
-- ============================================================================
...
SELECT DISTINCT ON (ppd.id) ppd.id AS det_id,
   ...
   FROM pedido_proveedor_detalle ppd
     ...
     LEFT JOIN LATERAL ( SELECT pl2.lpn,
         ...
         LIMIT 1) pl ON true
  -- *** CAMBIO CRÍTICO: Filtrar por estado_transito EN_TRANSITO ***
  WHERE pp.estado_transito = 'EN_TRANSITO'
    AND GREATEST(0, COALESCE(ppd.cantidad_pares, 0) - COALESCE(ppd.pares_vendidos, 0)) > 0
  ORDER BY ppd.id;

-- [MIGRATION-109] CRÍTICO: Bloquear PP en depósito de aparecer en v_stock_rimec