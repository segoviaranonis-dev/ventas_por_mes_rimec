-- MIG-090 — Fix: manejar facturas duplicadas en descuentos_lote
-- Bug: si hay múltiples facturas con mismo pp_id+marca+caso, la subconsulta retorna múltiples filas
-- Fix: usar DISTINCT ON para tomar solo la primera configuración de cada factura
...
BEGIN
  ...
  -- FIX MIG-090: usar DISTINCT ON para evitar duplicados en facturas_config
  WITH facturas_config AS (
    ...
    ORDER BY pp_id, marca, caso, (f->>'items_count')::int DESC  -- Priorizar factura con más items
  ),
  ...
  -- FIX MIG-090: usar DISTINCT ON para evitar duplicados
  WITH facturas_config AS (
    SELECT DISTINCT ON (pp_id, marca, caso)
      (f->>'pp_id')::bigint AS pp_id,
      f->>'marca' AS marca,
...
COMMENT ON FUNCTION public.carrito_validar(bigint) IS
  'MIG-090: fix duplicados - usa DISTINCT ON para evitar error "more than one row"';

COMMIT;

SELECT 'MIG-090 OK: validar maneja facturas duplicadas' AS estado;
