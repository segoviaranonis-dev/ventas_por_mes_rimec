-- MIG-087 — confirmar_pedido_web: eliminar validación de precios (ya validó carrito_validar)
-- Problema: confirmar valida precios contra precio_lpn fijo, genera falsos positivos con multi-lista
-- Solución: confiar en token de carrito_validar, solo validar STOCK en confirmar
  ...
  FOR v_lote IN SELECT * FROM jsonb_array_elements(p_payload->'lotes') LOOP
    ...
    FOR v_factura IN SELECT * FROM jsonb_array_elements(v_lote->'facturas') LOOP
      ...
      FOR v_item IN SELECT * FROM jsonb_array_elements(v_factura->'items') LOOP
        ...
        -- MIG-087: Eliminada validación de precios (ya validó carrito_validar con token)
        -- Solo validamos STOCK crítico para no sobrevender
...
COMMENT ON FUNCTION public.confirmar_pedido_web IS
  'MIG-087: confía en token de validación, solo valida STOCK (no precios) al confirmar';

COMMIT;

SELECT 'MIG-087 OK: confirmar sin validar precios' AS estado;
