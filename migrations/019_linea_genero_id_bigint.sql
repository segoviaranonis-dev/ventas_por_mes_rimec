-- ============================================================================
-- 019 · Promover linea.genero_id a bigint (alineado con genero.id) y recrear
--      v_stock_web para que el JOIN sea homogéneo.
-- ----------------------------------------------------------------------------
-- Motivo: la 018 dejó genero_id como integer. La maestra public.genero usa
-- bigint en su PK, y el JOIN gen ON gen.id = l.genero_id queda con casteo
-- implícito. Se promueve el tipo y se refresca la vista.
-- ============================================================================

BEGIN;

-- 1) Tirar dependencia (vista) para poder cambiar el tipo.
DROP VIEW IF EXISTS public.v_stock_web;

-- 2) Cambiar tipo a bigint conservando los valores existentes.
ALTER TABLE public.linea
    ALTER COLUMN genero_id TYPE bigint USING genero_id::bigint;

-- 3) Recrear la vista catálogo web con los nombres ya unificados.
