-- MIG-139 — confirmar_pedido_web: marca_id 0 / inválido → NULL (FK marca_v2)
-- Fix: factura_interna_marca_id_fkey — PE vista v_stock_pe_rimec usa COALESCE(l.marca_id, 0)
-- Base: MIG-138
  ...
  FOR v_lote IN SELECT * FROM jsonb_array_elements(p_payload->'lotes') LOOP
    ...
    FOR v_factura IN SELECT * FROM jsonb_array_elements(v_lote->'facturas') LOOP
      ...
      FOR v_item IN SELECT * FROM jsonb_array_elements(v_factura->'items') LOOP
        ...
        v_is_pe_item := v_det_id IS NOT NULL AND v_det_id >= c_pe_det_base;
...
COMMENT ON FUNCTION public.confirmar_pedido_web IS
  'MIG-139: PE sin FK pp_id · marca_id saneado · stock PE';

COMMIT;

SELECT 'MIG-139 OK: confirmar_pedido_web marca_id FK' AS estado;
