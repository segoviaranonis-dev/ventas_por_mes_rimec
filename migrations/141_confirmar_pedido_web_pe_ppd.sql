-- MIG-141 — confirmar_pedido_web: PE Alejandro Magno vía ppd (v_stock_pe_rimec)
-- Fix: MIG-140 buscaba stock_pronta_entrega_rimec — PE real usa pedido_proveedor_detalle.id
-- Base: MIG-140
    ...
    FOR v_factura IN SELECT * FROM jsonb_array_elements(v_lote->'facturas') LOOP
      ...
      FOR v_item IN SELECT * FROM jsonb_array_elements(v_factura->'items') LOOP
        ...
        IF v_is_pe_ppd AND v_precio_payload IS NOT NULL THEN
          SELECT vs.lpn INTO v_precio_bd_lista
          ...
          IF v_precio_bd_lista IS DISTINCT FROM v_precio_payload THEN
            RAISE EXCEPTION 'Precio cambió para PE ppd % (LPN): payload %, BD %.',
              v_det_id, v_precio_payload, v_precio_bd_lista;
          END IF;
...
COMMENT ON FUNCTION public.confirmar_pedido_web IS
  'MIG-141: PE Alejandro Magno ppd · staging 800M+ · tránsito CP';

COMMIT;

SELECT 'MIG-141 OK: confirmar_pedido_web PE ppd' AS estado;
