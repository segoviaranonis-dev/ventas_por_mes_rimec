-- MIG-085 — carrito_validar recalcula precios automáticamente antes de validar
-- Simplifica UX: usuario solo hace VALIDAR → CONFIRMAR (sin botón "Ver Totales")
-- Proceso:
--   1. Lee configuración de cada factura (lista_precio_id, descuentos)
--   2. Recalcula precio_snapshot de todos los items según su factura
--   3. Valida stock (no precio, porque ya recalculó)
--   4. Emite token si stock OK
...
BEGIN
  ...
  -- PASO 2: Validar solo STOCK (precios ya están actualizados)
  WITH detalle AS (
    SELECT
      ci.det_id,
...
COMMENT ON FUNCTION public.carrito_validar(bigint) IS
  'MIG-085: recalcula precios automáticamente antes de validar (simplifica UX).';

COMMIT;

SELECT 'MIG-085 OK: validar con recalculo automatico' AS estado;
