-- MIG-091 — Fix: conversión de descuentos null desde JSONB
-- Bug: (f->'descuentos'->0)::text::numeric falla cuando el valor JSON es null
-- porque ::text convierte JSON null a la cadena "null"
-- Fix: usar NULLIF o validar antes de ::numeric
...
BEGIN
  ...
  -- FIX MIG-091: manejar descuentos null correctamente
  ...
  -- FIX MIG-091: usar ->> para obtener texto directamente (evita "null" string)
  WITH facturas_config AS (
    SELECT DISTINCT ON (pp_id, marca, caso)
      (f->>'pp_id')::bigint AS pp_id,
      f->>'marca' AS marca,
...
COMMENT ON FUNCTION public.carrito_validar(bigint) IS
  'MIG-091: fix descuentos null - usa ->> para evitar conversion "null" string';

COMMIT;

SELECT 'MIG-091 OK: validar maneja descuentos null correctamente' AS estado;
