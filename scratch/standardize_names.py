...
SQL_CHANGES = [
    # 1. Estandarización de producto_v2
    "ALTER TABLE producto_v2 RENAME COLUMN linea TO linea_id;",
    
    # 2. Estandarización de v_stock_web
    """
    CREATE OR REPLACE VIEW v_stock_web AS
    WITH mov_agg AS (
             SELECT md.combinacion_id,
                m.id_marca_ref,
                sum(md.cantidad) AS stock_web
               FROM (movimiento_detalle md
                 JOIN movimiento m ON ((m.id = md.movimiento_id)))
              WHERE ((m.deposito_id_destino = 7) AND (m.estado = 'CONFIRMADO'::text))
              GROUP BY md.combinacion_id, m.id_marca_ref
            )
     SELECT agg.combinacion_id,
        mv.descp_marca AS descp_marca, -- Estandarizado
        l.id AS linea_id,
        ...
        c.color_id,
        col.descripcion AS descp_color,
        col.hex_web,
        ( SELECT m_f9.id_material_f9
               FROM material_v2 m_f9
              WHERE ((m_f9.id_marca = agg.id_marca_ref) AND ((m_f9.cod_material)::text = (mat.codigo_proveedor)::text))
             LIMIT 1) AS id_material_f9,
        ( SELECT c_f9.id_color_f9
               FROM color_v2 c_f9
              WHERE ((c_f9.id_marca = agg.id_marca_ref) AND ((c_f9.cod_color)::text = (col.codigo_proveedor)::text))
    ...
    # 3. Estandarización de v_stock_rimec
    """
    ...
    SELECT 
        ...
        COALESCE(mv.descp_marca, '—') AS descp_marca, -- Estandarizado
        ...
        COALESCE(d.descp_material, d.descp_material, '') AS descp_material, -- Ya era descp_material
        COALESCE(d.color_code, '') AS color_code,
        COALESCE(d.descp_color, d.descp_color, '') AS descp_color, -- Ya era descp_color
        d.grades_json,
